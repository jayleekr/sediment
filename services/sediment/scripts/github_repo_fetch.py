...
def _render_ingest_sources_doc(integration: dict) -> str:
    """Render the self-describing vault source document for one tenant.

    The document is generated from integration config, not inferred from chat
    retrieval, so "what is in this vault?" can be answered from ordinary
    citations after every scheduled sync.
    """
    cfg = dict(integration.get("config") or {})
    state = dict(cfg.get("state") or {})
    repos = _as_list(cfg.get("repos") or cfg.get("repo"))
    path_prefixes = _as_list(cfg.get("path_prefixes") or cfg.get("include_globs"))
    path_excludes = _as_list(cfg.get("path_excludes"))
    extensions = _as_list(cfg.get("extensions"))
    source_kind = cfg.get("source_kind") or integration.get("source_kind") or "vault"
    branch = cfg.get("branch") or "repository default"
    schedule = cfg.get("schedule") or "scheduler default"
    head_sha = state.get("head_sha") or ""
    last_sync_at = state.get("last_sync_at") or state.get("last_synced_at") or ""
    ...
    title = f"Ingest sources for tenant: {integration['slug']}"
    return f"""---
slug: ingest-sources
status: published
source: sediment-system
---

# {title}

This document is generated by Sediment after GitHub connector sync so the vault can describe its own evidence sources.

## Repositories

{bullets(repos, "No repositories configured")}

## Included paths

{bullets(path_prefixes, "All configured repository paths")}

## Excluded paths

{bullets(path_excludes, "No excluded paths configured")}

## File extensions

{bullets(extensions, "Connector default extensions")}
...
- Branch: `{branch}`
- Schedule: `{schedule}`
- Last recorded head: `{head_sha or "not recorded"}`
- Last sync time: `{last_sync_at or "not recorded"}`
"""


async def _publish_ingest_sources_doc(
    http: httpx.AsyncClient,
    integration: dict,
) -> tuple[bool, str]:
    return await _ingest_doc(
        ...
        ref="_meta/ingest-sources.md",
        body=_render_ingest_sources_doc(integration),
        doc_type="note",
    )
        ...
        if not dry_run:
            ...
            try:
                ...
                if final_heads:
                    ...
                    integration["config"] = {**cfg, "state": state}
                    summary["new_head"] = new_head
            except Exception as e:
                log.warning("github.watermark.update_failed", err=str(e)[:200])
                summary["watermark_error"] = str(e)[:200]

            ok, err = await _publish_ingest_sources_doc(http, integration)
            if ok:
                summary["meta_doc_ingested"] = True
            else:
                log.warning("github.meta_doc.ingest_failed", err=err)
                summary["ingest_failures"].append({
                    "ref": "_meta/ingest-sources.md",
                    "err": err,
                })
            ...
            try:
                await _record_vault_sync(integration["tenant_id"], {
                    ...
                    "meta_doc_ingested": summary["meta_doc_ingested"],
