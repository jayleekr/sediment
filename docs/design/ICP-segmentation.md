# Sediment ICP Segmentation v1.0

**Status:** Draft 2026-05-21. Locks the customer definition that drives engine architecture, connector priorities, pricing, and GTM.
...
> **Why now:** Before designing the Collection Engine v1 architecture, the ICP must be defined or we'll build for an imagined enterprise buyer we'll never reach. Jay's correction (2026-05-21): our real ICP is **data-pipeline-zero SMBs**, not Glean's customers.
...
- **Biggest architectural implication: KakaoTalk connector is P1**, not Slack
...
**Data landscape**
- KakaoTalk single most important channel: **patient comms + internal coordination + 원장 announcements**
- 진료차트 시스템 (electronic): 별도 솔루션 (Dr. NICE / 의사랑 / 메디칼라이즈) — Sediment 안 건드림
- 카카오워크 (rare, ~10% adoption)
- Paper meeting minutes for 월례회의 / 자체 교육
- Google Drive ad-hoc files
- WhatsApp / 카톡 with vendors (장비/소모품)
...
**Sediment value prop**
> "원장님 머리속의 운영 노하우를 위생사도 검색 가능한 자산으로 만듭니다"

핵심: **운영 SOP 자동 정리 + 인수인계 자료 자동 생성 + 환자 응대 사례 archive**
...
- 🔴 **KakaoTalk connector** (필수, 없으면 시작 불가)
- 🔴 **음성/사진 입력** (회의 녹음, 종이 회의록 사진)
- 🟡 SOP 자동 정리 strategy (`sop_capture` 신규 prompt)
- 🟡 환자 응대 사례 archive (`customer_record` 신규 prompt — 단 PII 자동 마스킹 필수)
- 🟢 의료법 26조 / PIPA 부합 (진료기록은 안 만짐, 운영 데이터만)
...
**Data landscape**
- KakaoTalk 100% (단톡방 5-20개)
- 종이 / 화이트보드 / 사장 노트
- 이메일 30% (거래처와만)
- Google Docs 가끔 (직원이 알아서 만든 거)
...
**Required product features**
- 🔴 KakaoTalk connector (절대 필수)
- 🔴 **30일 무료 체험** (Free tier보다 적극적)
- 🔴 음성 입력 (사장님 머릿속 → 음성으로 dump → 시스템화)
...
### 4.1 Connector priority (revised)

| Connector | A 치과 | B SMB | C 무파이프 | D Alumni | Priority |
|---|---|---|---|---|---|
| **KakaoTalk** | 🔴 essential | 🟡 important | 🔴 essential | 🟢 nice | **P1** |
...
→ **KakaoTalk이 P1**. 슬랙은 P2로 후순위. 이게 우리 차별점.

### 4.2 Distill strategy 신규 필요
...
| `customer_record` | 환자/거래처 응대 히스토리 정리 (PII 마스킹 필수) | A, B |
| `voice_dump` | 사장님 음성 메모 → 구조화된 결정/지시사항 | C |
| `paper_minutes` | 종이 회의록 사진 → 디지털 결정/액션 | A |
| `vendor_thread` | 거래처와의 협상/약속 히스토리 | B, C |
...
2. **Connector framework: KakaoTalk 최우선**, Discord/Slack/Notion 다음. GitHub/Email/Drive는 P3-4.
3. **White-glove onboarding 지원**: bulk import, manual seed, fixture mode 강화. self-serve는 보조.
4. **Voice + 사진 입력 필요**: text-only assumption 깨야 함 (C archetype + A 종이 회의록).
5. **PII 마스킹 자동화**: redact_pii.yaml prompt를 propose → execute로 격상.
6. **한국 PIPA 부합 audit log**: 어떤 데이터 누가 언제 봤는지 (worksheet 1개로 충분).
7. **가격 enforcement는 quota-driven, not seat-driven**: events/mo + connectors + history days.
8. **3-layer RBAC 단순화**: 사장 (admin) / 팀장 (manager) / 직원 (member). SAML 패스.
9. **Scheduler: APScheduler in-process 충분 Y1**. Celery 도입은 50+ tenant 후.
10. **Compliance: 한국 PIPA만**. SOC 2 / GDPR 패스 Q4 2027까지.

---

## 7. Open Decisions (Jay 결정 필요)

| # | 질문 | 옵션 |
|---|---|---|
| 1 | Q3 2026 beachhead 합계 N tenant target | 5 / 7 / 10 / 다른 숫자 |
| 2 | Starter ₩49K tier 추가? | (a) 추가 / (b) Solo $19로 충분 / (c) 가격 검증 후 결정 |
| 3 | KakaoTalk connector 구현 우선순위 | (a) Q3 안에 MVP / (b) Q4로 미루고 Discord로만 시작 |
