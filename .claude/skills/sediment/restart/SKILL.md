...
[ ! -f "$STATE" ] && { echo "no STATE.json — run /sediment:start first"; exit 1; }
...
| `stalled_5_iter` | Run `/sediment:medic` first to diagnose stall, then restart with `--resume`. |
| `max_iter_reached` | Run `/sediment:medic`, examine TODO, then restart fresh OR with `--resume`. |
...
If anything is down: invoke `/sediment:resurrect all` first.
...
```
Resumed Ralph at iter=N (phase=PX). Supervisor pid=M. Watch with /sediment:status.
```
