...
```
/sediment:kill [--keep-services]   # default: stop services too
```
...
echo "STOP — manual /sediment:kill at $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
...
if [ "${1:-}" != "--keep-services" ]; then
  for pid_f in output/bootstrap/{ingester,metadata,sediment_platform,sediment_langgraph}.pid; do
