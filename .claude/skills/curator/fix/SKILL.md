...
Dispatch `curator-fixer` to drain the latest work-order.
...
If WO empty: tell user there's nothing to fix; suggest `/curator:status` or `/curator:medic`.
