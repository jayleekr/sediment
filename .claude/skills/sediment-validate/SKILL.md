...
```
/sediment-validate <phase> [mode] [focus]
...
Examples:
- `/sediment-validate p1` — single-shot P1
- `/sediment-validate p2 loop` — 50-iter convergence loop on P2
- `/sediment-validate p1 single rls` — only run L3 (RLS) layer in P1
- `/sediment-validate p1 loop rag` — loop, but only RAG layer
...
|---|---|---|
| `single` | `sediment-validator` | one rubric pass, return report |
| `loop` | `sediment-loop-orchestrator` | 50-iter with auto-fix + specialist dispatch |
...
For `mode=single`:
```
Task tool: subagent_type=sediment-validator
prompt:
  Phase: {phase}
  Mode: single
...
For `mode=loop`:
```
Task tool: subagent_type=sediment-loop-orchestrator
prompt:
  Phase: {phase}
  max_iter: 50
  target_pct: 95
...
- Never bypass `sediment-validator`'s pre-flight check — services must be up before
  the rubric runs.
...
1. Copy `.claude/agents/sediment-*.md` to the target project's `.claude/agents/`.
2. Copy `.claude/skills/sediment-validate/` to the target project's `.claude/skills/`.
