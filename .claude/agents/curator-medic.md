...
name: curator-medic
description: >
  Curator Medic — self-healing agent. Runs every N Ralph iterations (or on demand)
...
>     "stalled" entries OR when /curator:medic is invoked.
...
**Recovery**: dispatch `curator-fixer` with the matching health-check id.
...
**Recovery**:
- If recipe exists for the underlying check id → dispatch `curator-fixer`
...
- **Code modification policy**: you don't patch yourself. If diagnosis identifies
  a clear TIER-2 root cause (recurring same bug), dispatch `curator-coder` with a
