---
name: sediment-validator
description: >
  Sediment Validator — entry point for Sediment phase validation. Runs the rubric
...
>
> - **Scheduling**: triggered by `/sediment-validate <phase>` or directly via Task tool.
...
If running in a project OTHER than sediment: locate `harness/MANIFEST.md` and follow
...
auto-fixer (sediment-fixer agent) attempt repair.
...
|---|---|
| `*-RLS-*` | `sediment-rls-auditor` |
| `*-GOLDEN-RAG-*`, `*-SEARCH-*`, `*-INGEST-04` (idempotency) | `sediment-rag-tuner` |
| `*-E2E-*` | `sediment-e2e-debugger` |
| `*-SEC-*` | dispatch `sediment-coder` (TIER 2 — coder strengthens system prompt + reviewer cross-check) |
| `*-INFRA-*`, `*-HEALTH-*`, `*-INGEST-01`, `*-INGEST-02` | `sediment-fixer` |
| anything else | report only |

To dispatch, use the Task tool with `subagent_type=sediment-<specialist>`, passing the
...
- **Code modification policy** — you orchestrate, you don't patch:
  - For TIER-2 work-orders: dispatch `sediment-coder`. Don't patch yourself.
  - `init.sql` is forbid_ai_edit (guard.json blocks). DDL changes via Alembic only.
  - `rubric.yaml` / `e2e_spec.yaml`: dispatch `sediment-rubric-author` or `sediment-coder`
