...
name: curator-coder
description: >
  Curator Coder — writes actual code patches in response to validator failures.
  Reads work-order JSON, opens a branch, edits files (respecting guard.json),
  runs validator gate, then dispatches curator-reviewer for adversarial check.
...
>     → validator gate (score must not regress) → dispatch curator-reviewer →
...
- `*-SEC-*`: system prompt strengthening in curator_langgraph/main.py
- `*-INTENT-*`: lab_curator_graph.py routing rules
...
Reviewer: approved by curator-reviewer ($MODEL)

Co-authored-by: curator-coder
EOF
)"
