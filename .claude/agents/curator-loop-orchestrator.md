---
name: curator-loop-orchestrator
description: >
  Curator Loop Orchestrator — top-level driver for the 50-iteration self-improving
  loop. Coordinates curator-validator, curator-fixer, and specialists. Decides when
...
# Curator Loop Orchestrator

> SSL Skill Manifest
>
> - **Scheduling**: invoked by `/curator-validate` slash command in `loop` mode, or
...
```json
{
  ...
  "specialists_dispatched": ["curator-rag-tuner@iter-08", "curator-fixer@iter-12"],
...
If any fail: dispatch `curator-fixer` once with bootstrap recipes.
...
- E2E flakes spiking? → curator-e2e-debugger
...
| max_iter_reached | dispatch specialist → if they identify TIER-2 fix, dispatch `curator-coder` to apply (auto-commit on reviewer approval). Only escalate to human if reviewer rejects 2 attempts. |
...
- **Code modification policy**: you orchestrate, never patch yourself. Dispatch
  `curator-coder` for TIER-2 work-orders (`recipes.yaml > ai_propose_review_commit`).
