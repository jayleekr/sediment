...
name: curator-fixer
description: >
  Curator Fixer — applies declarative auto-fix recipes from harness/recipes.yaml in
...
> SSL Skill Manifest
>
> - **Scheduling**: invoked by curator-validator when failures match recipe patterns.
...
- **Code modification policy** (see `recipes.yaml` 4-tier):
  ...
  - TIER 2 `ai_propose_review_commit`: dispatch `curator-coder` via Task tool with
