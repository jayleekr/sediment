---
name: curator-rubric-author
description: >
  Curator Rubric Author — meta-agent that authors NEW rubric.yaml entries when a
...
async def check_decision_extract_precision(spec: dict, **_) -> dict:
    """TODO(curator-rubric-author): implement. See TEST_REQUIREMENTS.md §L7.3"""
...
- **Code modification policy** (TIER 2 — `ai_propose_review_commit`):
  ...
  - To APPLY: dispatch `curator-coder` with proposal as work-order. Coder
