...
- [ ] blocked-by:P0.seed P0.validate: invoke `sediment-validator` subagent for phase P0 single-shot → must converge
...
- [ ] blocked-by:P1.ingest P1.validate: `/sediment-validate p1 loop` → converge
- [ ] blocked-by:P1.validate P1.golden-tune: if recall@3 < 80%, dispatch sediment-rag-tuner
...
- [ ] blocked-by:P2.web P2.validate: `/sediment-validate p2 loop`
- [ ] blocked-by:P2.validate P2.e2e-stable: if E2E flake > 5%, dispatch sediment-e2e-debugger

## Phase 3 — Automation (optional for solo MVP)

- [ ] blocked-by:P2.validate P3.validate: `/sediment-validate p3 single` (cron + GHA + Discord)

## Code-mode tasks (AI-driven fixes — runs whenever validator score < 95%)

- [ ] CODE.always: Per iter, if any TIER-2 work-order exists in latest output/validation/, dispatch sediment-coder with that work-order. Coder + reviewer + auto-commit. NEVER dispatch coder for TIER-3 (RLS) or TIER-4 (guard.json paths).
...
- [ ] CODE.review: Coder calls Task subagent_type=sediment-reviewer with the branch. If verdict=approve, commit. If revise, ONE more attempt. If reject, write LEARNINGS + escalate.
